function [P, P0, output] = cp_wopt_reg(Z,W,R,lamda1,varargin)
...
if (isequal(funtype,'auto') && isa(Z,'tensor')) || isequal(funtype,'default')
    funhandle = @(x) tt_cp_wfun_reg(Z,W,x,normZsqr,lamda1);  %%% change by Lu
else
    ...
    funhandle = @(x) tt_cp_wfun_reg(Zvals,W,x,normZsqr,lamda1,fflag); %%%change by Lu
end
...
function [f,G] = tt_cp_wfg_reg(Z,W,A,normZsqr,lamda1) %%%% change by Lu
...
% function value
f = 0.5 * normZsqr - innerprod(Z,B) + 0.5 * norm(B)^2+lamda1/2*f_4;%change by Lu
...
for n = 1:N
    G{n} = zeros(size(A{n}));
    G{n} = -mttkrp(T,A,n)+lamda1*A{n};%change by Lu
end

function [f,g] = tt_cp_wfun_reg(Zdata,W,x,normZsqr,lamda1,memflag)
...
if isa(Zdata,'tensor') || isa(Zdata,'sptensor')
    ...
    [f,G] = tt_cp_wfg_reg(Zdata,W,A,normZsqr,lamda1); %%% change by Lu
else
    if ~exist('normZsqr','var')
    ...
    [f,G] = tt_cp_wfg_sparse_reg(Zdata,W,A,normZsqr,memflag,lamda1);
end
...
function [f,G] = tt_cp_wfg_sparse_reg(Zvals,W,A,normZsqr,memflag,lamda1)
...
f = 0.5 * normZsqr - Zvals'*Bvals + 0.5 * sum(Bvals.^2)+ 0.5 * norm(B)^2+lamda1/2*f_4;%change by Lu
...
for n = 1:N
    G{n} = -G{n}+lamda1*A{n};%change by Lu
end
